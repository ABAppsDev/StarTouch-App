...
class OrderTakeAwayScreenModel(
...
) : BaseScreenModel<OrderTakeAwayState, OrderTakeAwayUiEffect>(OrderTakeAwayState()),
    OrderTakeAwayInteractionListener {
    override val viewModelScope: CoroutineScope get() = screenModelScope

    init {
        orders.clear()
        updateState { it.copy(orderTakeAwayItemState = orders.toList()) }
        getAllPresets()
        orders.addAll(items.map { it.toState() })
        val newList = orders.toList()
        updateState { it.copy(orderTakeAwayItemState = newList) }
    }
    ...
    fun abortedCheck(checkId: Long) {
        tryToExecute(
            ...
            onSuccess = {
                updateState { it.copy(orderTakeAwayItemState = emptyList()) }
    ...
    override fun onClickItemModifier(name: String) {
        ...
        item?.let {
            ...
            updateState { it.copy(orderTakeAwayItemState = orders.toList(), qty = 0f) }
        }
        updateState {
            it.copy(
                selectedPresetId = 0,
    ...
    fun addInExistItem() {
        ...
        updateState {
            it.copy(
                orderTakeAwayItemState = newList,
    ...
    fun add() {
        ...
        item?.let {
            ...
            updateState {
                it.copy(
                    orderTakeAwayItemState = orders.toList(),
                    warningItemIsVisible = false,
                    isPresetVisible = true,
                )
            }
    ...
    override fun onClickFire() {
        ...
        orders.forEachIndexed { i, order ->
            if (order.isModifier) {
                ...
                updateState {
                    it.copy(
                        orderTakeAwayItemState = newList.toList()
                    )
                }
            }
        }
        tryToExecute(
            function = {
                val fireItems =
                    state.value.orderTakeAwayItemState.filter { !it.fired }.map { it.toEntity() }
    ...
    override fun addItem(orderItemState: OrderTakeAwayItemState) {
